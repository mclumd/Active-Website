...
handle_contra(New, Old, []):-
	...
	(contra_distrust_descendants(true) -> 
        ...
        (is_an_old_node(Old) -> 
	     (unindex(Old, OName),
	      assert(distrusted_forms([Old|DF])));	  % this is deleted
	 (new_to_node(Old, OLDNEW), 
	  assert(OLDNEW), 
	  unindexnewone([OF], OName),
	  assert(distrusted_forms([OLDNEW|DF])))),	  % delete in this case
	...
	retract(distrusted_forms(DF2)),
	assert(distrusted_forms([NEWOLD|DF2])),	  % this is deleted too
